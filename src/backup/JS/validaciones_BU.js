...
function validarCamposFormulario() {
    ...
    if (!regexLetrasNumerosEspacios.test(nombreProducto)) {
        alert('El campo "Producto" solo puede contener letras, números, espacios y caracteres acentuados.');
        return false;
    }

    // 🔍 Validación de campo "Precio Unidad" (obligatorio y >= 0)
    if (!precioUnidad || parseFloat(precioUnidad) < 0) {
        alert('El campo "Precio Unidad" es obligatorio y debe ser un número mayor o igual a 0.');
        return false;
    }
    ...
    if (marcaProducto && !regexLetrasNumerosEspacios.test(marcaProducto)) {
        alert('El campo "Marca" solo puede contener letras, números, espacios y caracteres acentuados.');
        return false;
    }
    ...
    if (!regexLetrasNumerosEspacios.test(supermercado)) {
        alert('El campo "Supermercado" solo puede contener letras, números, espacios y caracteres acentuados.');
        return false;
    }
...
async function asignarIDDisponible() {
    const idsExistentes = productosCache.map(p => parseInt(p._id)).filter(Number.isInteger);
    const idsDisponibles = Array.from({ length: Math.max(...idsExistentes, 0) + 1 }, (_, i) => i + 1).filter(id => !idsExistentes.includes(id));
    return idsDisponibles.length > 0 ? idsDisponibles[0].toString() : (idsExistentes.length + 1).toString();
}
