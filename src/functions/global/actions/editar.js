...
import { openModal } from "../modals/cerrar.js"; 
...
export async function editarProducto(id) {
  try {
    ...
    const [prodRes, precioRes, supersRes, provRes] = await Promise.all([
      ...
      fetch(`${ENDPOINTS.precios}/producto/${id}`),   // /api/precios/producto/:id (alias)
    ...
    const sup  = supermercados.find(s => String(s._id) === String(producto.Supermercado_id)) || {};
    const prov = proveedores.find(p => String(p._id) === String(producto.Proveedor_id)) || {};

    // ----- Producto (campos directos)
    ...
    // ----- “Descripción” (del propio producto)
    ...
    safeSetValue(
      "edit-ingredientes",
      Array.isArray(producto.Ingredientes) ? producto.Ingredientes.join(", ") : ""
    );

    // ----- Precios
    if (precioData) {
      safeSetValue("edit-precio", precioData.precioActual ?? "");
      // mejor dejar vacío en inputs numéricos si no hay dato:
      safeSetValue("edit-unidadLote", Number.isFinite(precioData.unidadLote) ? precioData.unidadLote : "");
      safeSetValue("edit-precioPorUnidad",
        Number.isFinite(precioData.precioUnidadLote) ? precioData.precioUnidadLote : ""
      );
      ...
      safeSetValue("edit-unidadLote", "");
      safeSetValue("edit-precioPorUnidad", "");
      safeSetValue("edit-precioHistorico", "");
    }

    // ✅ Abrir modal correctamente (sin tocar display/aria a mano)
