...
import {
  ...
  cargarNav,
  renderizarProductos,
} from "../functions/global/funciones.js";
...
async function editarProducto(id) {
  try {
    ...
    const descripcion = await (
      await fetch(`http://localhost:3000/api/descripcion/producto/${id}`)
    ).json();
    const supermercados = await (
      await fetch(`http://localhost:3000/api/supermercados`)
    ).json();
    const proveedores = await (
...
async function guardarProductoNuevo() {
  try {
    ...
    const subtipo = await procesarCampoNuevo(
      "add",
      "subtipo",
      insertarNuevoSubtipo
    );
    const proveedor = await procesarCampoNuevo(
      "add",
      "proveedor",
      async (nombre) => {
        const pais =
          document.getElementById("add-pais-proveedor").value || "España";
        return await insertarNuevoProveedor(nombre, pais);
      }
    );
    const supermercado = await procesarCampoNuevo(
      "add",
      "supermercado",
      async (nombre) => {
    ...
    formData.append(
      "unidadPeso",
      document.getElementById("add-unidadPeso").value
    );
    ...
    formData.append(
      "precioDescuento",
      document.getElementById("add-precioDescuento")?.value || ""
    );
    formData.append(
      "unidadLote",
      document.getElementById("add-unidadLote")?.value || "N/A"
    );
    formData.append(
      "precioPorUnidad",
      document.getElementById("add-precioPorUnidad")?.value || ""
    );
    ...
    // 📈 Precio Histórico
    const precioHistoricoTexto =
      document.getElementById("add-precioHistorico")?.value || "";
    ...
    const response = await fetch(
      "http://localhost:3000/api/productos-completos",
      {
        method: "POST",
        body: formData,
      }
...
async function guardarCambiosDesdeFormulario() {
  try {
    ...
    const supermercadoNombre = await procesarCampoNuevo(
      "edit",
      "supermercado",
      async (nombre) => {
        return await insertarNuevoSupermercado(nombre, ubicaciones);
    ...
    formData.append(
      "utilidad",
      document.getElementById("edit-utilidad")?.value || "Sin descripción"
    );
    formData.append("ubicaciones", JSON.stringify(ubicaciones));
    formData.append(
      "ubicacion",
      document.getElementById("edit-ubicacion-super")?.value || ""
    );
    formData.append(
      "ciudad",
      document.getElementById("edit-ciudad-super")?.value || ""
    );
    formData.append(
      "paisSupermercado",
      document.getElementById("edit-pais-super")?.value || "España"
    );
    formData.append(
      "paisProveedor",
      document.getElementById("edit-pais-proveedor")?.value || "España"
    );
    ...
    formData.append(
      "unidadLote",
      document.getElementById("edit-unidadLote")?.value || ""
    );
    formData.append(
      "precioPorUnidad",
      document.getElementById("edit-precioPorUnidad")?.value || ""
    );
    ...
    const productoRes = await fetch(
      `http://localhost:3000/api/productos-completos/${id}`,
      {
        method: "PUT",
        body: formData,
      }
    ...
    const historialTexto = document.getElementById(
      "edit-precioHistorico"
    ).value;
    let historialArray = [];
    ...
    await fetch(`http://localhost:3000/api/precios/por-producto/${id}`, {
      ...
      body: JSON.stringify({
        ...
        precioDescuento:
          document.getElementById("edit-precioDescuento").value || null,
        unidadLote: document.getElementById("edit-unidadLote").value || "N/A",
        precioUnidadLote: parseFloat(
          document.getElementById("edit-precioPorUnidad").value || "0"
        ),
