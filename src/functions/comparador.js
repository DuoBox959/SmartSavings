...
async function cargarTiendasAdicionales(selector) {
    ...
    if (!productoSeleccionado) {
        selector.innerHTML = '<option value="">Selecciona un producto primero</option>';
        return;
    }
...
async function compararPrecios() {
    ...
    if (new Set(tiendasSeleccionadas).size !== tiendasSeleccionadas.length) {
        resultadoDiv.innerHTML = '<p style="color:red;">Por favor, selecciona tiendas diferentes.</p>';
        return;
    }
    ...
    try {
        ...
        const tiendasMasBaratas = precios.filter(p => p.precioUnidad === precioMasBarato);

        let resultadoHTML = `<h3>Comparación de Precios para "${productoSeleccionado}"</h3><ul>`;
        precios.forEach(({ tienda, precioUnidad, precioLote, peso, unidadPeso }) => {
            resultadoHTML += `<li>${tienda}: ${precioUnidad.toFixed(2)} € (Unidad), ${precioLote} € (Lote), Peso: ${peso} ${unidadPeso}</li>`;
        });
        resultadoHTML += '</ul>';

        if (tiendasMasBaratas.length === 1) {
            resultadoHTML += `<p>La tienda más barata es <strong>${tiendasMasBaratas[0].tienda}</strong> con un precio de <strong>${tiendasMasBaratas[0].precioUnidad.toFixed(2)} €</strong>.</p>`;
        } else {
            resultadoHTML += `<p>Las tiendas más baratas son:</p><ul>`;
            tiendasMasBaratas.forEach(({ tienda, precioUnidad }) => {
                resultadoHTML += `<li>${tienda}: <strong>${precioUnidad.toFixed(2)} €</strong></li>`;
            });
            resultadoHTML += '</ul>';
        }
